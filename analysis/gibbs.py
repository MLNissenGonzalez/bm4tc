...
# the same attack budgets `sweep.py` uses, then purify them with class-marginalized
...
# travelled up to k × step_delta_rel × (hi − lo). Purification strength is set by the
...
# RELATIVE budgets: fractions of the input domain width (see "Budget vocabulary"
# in CLAUDE.md). Legendre (width 2.0): 0.05/0.1/0.15 -> abs 0.1/0.2/0.3.
...
GIBBS = {
    ...
    "n_sweeps": [1, 3, 6],
    # Candidate values per feature. With a step_delta_rel these all land *inside*
...
_cli.add_argument("--step-delta-rel", type=float, default=None,
                  help="Per-sweep L-inf purification step, as a fraction of the "
                       "input domain width.")
...
_cli.add_argument("--eps-rel", default=None,
                  help="Comma-separated attack budgets as FRACTIONS of the "
                       "input domain width, e.g. '0.05,0.1,0.15'.")
...
if _cli_args.step_delta_rel is not None:
    GIBBS["step_delta_rel"] = _cli_args.step_delta_rel
...
if _cli_args.eps_rel is not None:
    EPS_REL = [float(s) for s in _cli_args.eps_rel.split(",") if s.strip()]
...
# Budgets stay RELATIVE throughout; each is converted to absolute only where the
# attack is invoked, matching analysis/run.py and analysis/sweep.py.
ATTACK["eps_rel"] = list(EPS_REL)
...
print(f"Attack: {ATTACK['method']} L{ATTACK['norm']} steps={ATTACK['num_steps']}  "
      f"eps_rel={EPS_REL} "
      f"(abs {[round(s * _RANGE_SIZE, 6) for s in EPS_REL]})")
print(f"Gibbs:  sweeps={SWEEP_POINTS}  step_delta_rel={GIBBS['step_delta_rel']} "
...
def _generate_adversarial(attack, born, x, labels, eps_abs, device):
    """Batched PGD generation over an already-subsampled tensor.

    ``eps_abs`` is absolute (model-domain), converted by the caller.
    """
    out = []
    for i in range(0, len(x), EVAL_BATCH_SIZE):
        xb = x[i:i + EVAL_BATCH_SIZE].to(device)
        yb = labels[i:i + EVAL_BATCH_SIZE].to(device)
        out.append(attack.generate(born, xb, yb, eps_abs, device).detach().cpu())
...
def gibbs_analyze_run(run_dir: Path) -> dict:
    ...
    emitted by analysis/run.py (`gibbs_purify_acc/{eps_rel}/{k}` etc.) so downstream
    ...
    results["gibbs_step_delta_rel"] = GIBBS["step_delta_rel"]
    results["range_size"] = _RANGE_SIZE
    results["eps_unit"] = "rel"
    ...
    purifier = GibbsPurification(
        ...
        step_delta_rel=GIBBS["step_delta_rel"],
    )
    ...
    # 5. Per attack budget: attack, score undefended, purify, score again.
    attack = RobustnessEvaluation(
        method=ATTACK["method"],
        norm=ATTACK["norm"],
        criterion=CriterionConfig(name="nll", kwargs=None),
        eps_rel=ATTACK["eps_rel"],
        num_steps=ATTACK["num_steps"],
        random_start=True,
    )

    for eps_rel in ATTACK["eps_rel"]:
        eps_abs = rel_to_abs(eps_rel, _RANGE_SIZE)
        ek = fmt_budget(eps_rel)
        try:
            x_adv = _generate_adversarial(attack, cbm, x_test, y_test, eps_abs, device)
            adv_correct = _accuracy(cbm, x_adv, y_test, device)
            misclassified = ~adv_correct
            results[f"gibbs_adv_acc/{ek}"] = adv_correct.float().mean().item()
            results[f"gibbs_adv_log_px_mean/{ek}"] = _mean_log_px(cbm, x_adv, device)
        ...
        for k, (x_pur, log_px_after) in snapshots.items():
            try:
                ...
                results[f"gibbs_purify_acc/{ek}/{k}"] = correct_after.float().mean().item()
                results[f"gibbs_purify_recovery/{ek}/{k}"] = (
                    float((misclassified & correct_after).sum()) / n_misclf
                    if n_misclf > 0 else 1.0
                )
                results[f"gibbs_purify_log_px_mean/{ek}/{k}"] = float(log_px_after.mean())
            except Exception as e:
                logger.warning(f"Gibbs scoring failed (eps_rel={eps_rel}, k={k}): {e}; skipping")
                _recover_after_failure(cbm)
...
if run_dirs:
    _n_cond = 1 + len(ATTACK["eps_rel"])           # clean + one per epsilon
...
if not df.empty:
    ...
    with open(summary_path, "w") as f:
        ...
        f.write(f"Attack: {ATTACK['method']} L{ATTACK['norm']} "
                f"steps={ATTACK['num_steps']}  eps_rel={EPS_REL}\n")
        ...
        f.write(f"Gibbs: sweeps={SWEEP_POINTS}  step_delta_rel={GIBBS['step_delta_rel']} "
                f"(per-sweep L-inf step, NOT a global budget: after k sweeps the\n")
        f.write(f"       envelope is k x {GIBBS['step_delta_rel']} x range)  "
